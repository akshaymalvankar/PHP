...
//$policy->$id = htmlspecialchars(strip_args($data->$id));
$policy->name = htmlspecialchars(strip_tags($data->name));
$policy->amount = htmlspecialchars(strip_tags($data->amount));

if ($policy->name != '' && $policy->amount > 0) {
    if ($policy->insert()) {
        echo json_encode(
            array('success' => 'Record inserted successfully')
        );
    } else {
        echo json_encode(
            array('error' => 'Failed to insert record !')
        );
    }
}
